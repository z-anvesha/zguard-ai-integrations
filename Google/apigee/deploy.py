...
#: Shared with the SharedFlow bundle and both Cloud Run pipelines.
...
class ApigeeDeployer:
    ...
    def setup_kvm(self):
        ...
        # Create KVM. The name and key spelling match the SharedFlow and the
        # Cloud Run pipelines, so one provisioned environment serves any of them.
        resp = requests.post(
            base_url,
            headers=headers,
            json={"name": KVM_NAME, "encrypted": True},
        )
        if resp.status_code == 201:
            print(f"✓ Created encrypted KVM '{KVM_NAME}'")
        elif resp.status_code == 409:
            print(f"✓ KVM '{KVM_NAME}' already exists")
        else:
            print(f"⚠ KVM create returned {resp.status_code}: {resp.text}")

        entries = {
            "aiguard_api_key": self.aiguard_api_key,
            "aiguard_cloud": self.aiguard_cloud,
            "vertex_project": self.vertex_project,
            "vertex_model": self.vertex_model,
        }
        # Written only when explicitly pinned. Absent means the proxy calls
        # resolve-and-execute-policy and the API picks the policy for the key.
        if self.aiguard_policy_id:
            entries["aiguard_policy_id"] = self.aiguard_policy_id

        entries_url = f"{base_url}/{KVM_NAME}/entries"
